...
st.sidebar.write(get_balance(w3, account.address)) #need to check this
##########################################
...
if st.sidebar.button("Send Transaction"):
    ...
    def send_transaction(w3, account, candinate_address, wage):
    # Set gas price strategy
        w3.eth.setGasPriceStrategy(medium_gas_price_strategy)
    ...
    # Calculate gas estimate
        gasEstimate = w3.eth.estimateGas({"to": candinate_address, "from": account.address, "value": value})

    # Construct a raw transaction
        raw_tx = {
            "to": candinate_address,
            "from": account.address,
            "value": value,
            "gas": gasEstimate,
            "gasPrice": 0,
